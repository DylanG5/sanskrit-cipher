...
Jan.\ 18, 2026 & 1.3 & Added Section 5: Module Decomposition.\\
Jan.\ 18, 2026 & 1.4 & Added Sections 6--29: Module Interface Specifications for all modules.\\
Jan.\ 18, 2026 & 1.5 & Added Reflection and References.\\
Apr.\ 6, 2026 & 1.6 & Expanded the Section 2 exception glossary for consistency with the module specifications.\\
Apr.\ 6, 2026 & 1.7 & Removed the unimplemented \texttt{ML.TextSim} module from the MIS to align the document with the codebase.\\
Apr.\ 6, 2026 & 1.8 & Added explicit \texttt{pre:} and \texttt{post:} conditions to access routine semantics for stronger interface precision.\\
...
\begin{itemize}
    \item \textbf{AggregationError} — Score aggregation or multi-model fusion failure
    \item \textbf{AuthError} — Authentication or credential verification failure
    \item \textbf{CatalogError} — Catalog association lookup or persistence failure
    \item \textbf{ConfigError} — Missing or invalid configuration value
    \item \textbf{ExportError} — Export generation or serialization failure
    \item \textbf{FragmentError} — Fragment creation, update, or deletion failure
    \item \textbf{InboxOverflowError} — Too many pending authentication or request items to accept another
    \item \textbf{InvalidFormatError} — Uploaded file format or payload structure is not supported
    \item \textbf{LayoutError} — Canvas placement or rotation violates layout constraints
    \item \textbf{MatchError} — Fragment matching service or result generation failure
    \item \textbf{ModelError} — Machine learning inference, preprocessing, or feature extraction failure
    \item \textbf{NetworkError} — Network communication or backend connectivity failure
    \item \textbf{NotFoundError} — Requested resource does not exist or is not accessible in context
    \item \textbf{PreprocError} — Preprocessing job creation or execution failure
    \item \textbf{ProjectError} — Project creation, update, or query failure
    \item \textbf{SearchError} — Search query validation or execution failure
    \item \textbf{ServiceError} — Service-layer orchestration or downstream dependency failure
    \item \textbf{SessionError} — Session or saved-layout persistence failure
    \item \textbf{StorageWriteError} / \textbf{StorageReadError} — File or object storage I/O failure
    \item \textbf{UIRenderError} — User interface rendering or display update failure
    \item \textbf{UploadError} — File upload transfer failure
    \item \textbf{UserError} — User record creation or update failure
    \item \textbf{ValidationError} — Input, request, or argument validation failure
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Logical Expressions:} Represented using logical symbols such as $\land$ (and), $\lor$ (or), $\lnot$ (not), and the implication operator $\implies$.
\end{itemize}
...
\textbf{Machine Learning Modules (ML):} \texttt{ML.EdgeMatch}, \texttt{ML.ScriptClass}, \texttt{ML.Segmentation}, \texttt{ML.CircleRecognition}, \texttt{ML.LineCount}, \texttt{ML.EdgeClassification}
\end{minipage} \\
\addlinespace
Software Decision & 
\begin{minipage}[t]{0.7\linewidth}
...
This level hides operating-system and platform dependencies from the rest of the system. In this design, those low-level details remain external to the application modules rather than being specified as separate MIS leaf modules.

\subsubsection{Behaviour-Hiding Level}
This level defines the functional behavior of the system, implementing the requirements outlined in the SRS. It includes \texttt{HW.ImageStorage}, the user-facing modules (UI), the application logic (Service Layer), data persistence (Data Layer), and intelligent processing (ML Layer). These modules interact through clearly defined service interfaces, promoting modularity and separation of concerns.
...
\begin{itemize}
    ...
    \item \texttt{Util.Error} – for constructing storage-related error objects.
\end{itemize}
...
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any required identifiers, state, or external resources referenced by this routine are valid and available.
    \item post: if no exception is raised, the documented state transition is completed and the specified output is produced.
    \item transition: a candidate \texttt{ImageID} is generated and checked with \texttt{isUniqueImageID}; if the identifier is unused, the file is written to storage and \texttt{storageIndex} and \texttt{metadataCache} are updated with the new entry.
    \item output: a newly generated unique \texttt{ImageID}.
...
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any arguments or referenced identifiers used by this routine are valid.
    \item post: if no exception is raised, the specified output is produced and module state is unchanged unless stated otherwise below.
...
\begin{itemize}
    \item \texttt{generateImageID(file: ImageFile): ImageID} – generates a candidate identifier for a newly stored image.
    \item \texttt{isUniqueImageID(id: ImageID): bool} – returns \texttt{true} iff \texttt{id} is not already present in \texttt{storageIndex}.
    \item \texttt{validateFormat(file: ImageFile): bool} – checks that the uploaded file format is supported.
\end{itemize}
...
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any required identifiers, state, or external resources referenced by this routine are valid and available.
    \item post: if no exception is raised, the documented state transition is completed and the specified output is produced.
    \item transition: checks whether \texttt{Svc.AuthZ} is reachable, then sends credentials to it; updates \texttt{loginAttempts} and \texttt{authErrorMessage} based on response.
    \item output: \texttt{AuthResult} indicating success or failure.
    \item exception: \texttt{AuthError} if credentials rejected; \texttt{NetworkError} if the reachability check fails or the backend is unreachable.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{checkAuthServiceReachable(): bool} – checks whether the authentication service can be contacted before submitting credentials.
...
\subsection{Uses}
\begin{itemize}
    \item \texttt{Svc.API} – specifically \texttt{POST /fragments/upload} for submitting image files and receiving fragment identifiers, and \texttt{GET /fragments/\{id\}/status} for polling processing status after upload.
...
\texttt{placeFragment} & \texttt{id: FragmentID, position: Record\{x: float, y: float\}} & \texttt{none} & \texttt{LayoutError} \\
...
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any required identifiers, state, or external resources referenced by this routine are valid and available.
    \item post: if no exception is raised, the documented state transition is completed and the specified output is produced.
    \item transition: validates the \texttt{position} record, where \texttt{position.x} and \texttt{position.y} are the target canvas coordinates for the fragment's anchor point, then updates \texttt{currentState} with that new position for \texttt{id}.
    \item output: none.
...
\subsubsection{Local Functions}
\begin{itemize}
    \item \texttt{validateCanvasPosition(position: Record\{x: float, y: float\}): bool} – checks that a proposed placement is inside the canvas bounds.
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{startApplication} & \texttt{none} & \texttt{none} & \texttt{ServiceError} \\
...
\subsubsection{State Variables}
\begin{itemize}
    \item \texttt{appInitialized: bool} – true once handlers and service bindings are registered.
...
\noindent \texttt{startApplication()}:
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any required identifiers, state, or external resources referenced by this routine are valid and available.
    \item post: if no exception is raised, the documented state transition is completed and the specified output is produced.
    \item transition: initializes the route table, binds handlers to the application service boundary, and marks \texttt{appInitialized} as \texttt{true}.
    \item output: none.
    \item exception: \texttt{ServiceError} if the service boundary cannot be initialized.
\end{itemize}
...
\subsubsection{Local Functions}
\begin{itemize}
    \item \texttt{initializeRouteTable()} – registers the handlers used by the application entrypoint.
...
\subsection{Uses}
\begin{itemize}
    \item \texttt{ML.EdgeMatch}, \texttt{ML.Damage}.
...
\begin{itemize}
    ...
    \item \texttt{Util.Error} – for constructing \texttt{ModelError} exceptions.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
    \item \texttt{HW.ImageStorage} – for retrieving fragment images.
    \item \texttt{Util.Error} – for constructing \texttt{ModelError} exceptions.
\end{itemize}
...
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any arguments or referenced identifiers used by this routine are valid.
    \item post: if no exception is raised, the specified output is produced and module state is unchanged unless stated otherwise below.
    \item transition: writes informational log entry.
    \item output: none.
    \item exception: none.
\end{itemize}
...
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any arguments or referenced identifiers used by this routine are valid.
    \item post: if no exception is raised, the specified output is produced and module state is unchanged unless stated otherwise below.
    \item transition: writes warning log entry.
    \item output: none.
    \item exception: none.
\end{itemize}
...
\begin{itemize}
    \item pre: inputs satisfy the module assumptions, and any arguments or referenced identifiers used by this routine are valid.
    \item post: if no exception is raised, the specified output is produced and module state is unchanged unless stated otherwise below.
    \item transition: writes error entry including details from \texttt{err}.
    \item output: none.
    \item exception: none.
\end{itemize}
...
\begin{enumerate}
  ...
  requirements, hazard analysis, etc), it any, needed to be changed, and why?
    ...
    \hspace{2em} Very few changes were needed to our previous documents. Because this is a pre-approved project with a well-defined problem and scope, many of the requirements and expectations were already laid out for us in the project description. As a result, our earlier documents, such as the \href{https://github.com/DylanG5/sanskrit-ciphers-requirements}{SRS} and \href{https://github.com/DylanG5/sanskrit-cipher/blob/main/docs/HazardAnalysis/HazardAnalysis.pdf}{Hazard Analysis}, aligned naturally with the design work we did here. We did not need to revisit or adjust them significantly, beyond minor wording refinements and consistency updates.
...
Implementing the User Authorization Module (M8) was largely a smooth process due to the clarity of the provided MIS structure, though there were specific areas where ambiguity slowed down development. On the positive side, the Semantics section clearly defined the necessary state variables, making it immediately obvious that we needed \texttt{activeSessions} and \texttt{tokenExpiryTimes} as dictionary structures to manage session tokens and expiration timestamps. Additionally, the Exported Access Programs table was highly effective; having a concise checklist of inputs, outputs, and exceptions allowed us to scaffold the function signatures for \texttt{login}, \texttt{logout}, and \texttt{validateToken} very quickly. The inclusion of Exported Constants like \texttt{TOKEN\_EXPIRY\_MIN} was also beneficial, as it removed the need for magic numbers in the code and clearly indicated which features, such as refresh tokens, needed to be configurable.

However, we encountered challenges regarding data types and external integration. While the interface listed types like \texttt{Credentials} and \texttt{UserProfile}, the MIS did not define their internal structure, forcing us to make assumptions about the required fields (e.g., whether \texttt{Credentials} included just a password or also an API key). Furthermore, while the module specified a reliance on an External Authentication Provider and an \texttt{AUTH\_SERVER\_URL}, it lacked the specific API contract or protocol details required to implement the actual communication with that provider. There was also minor confusion regarding \texttt{SystemTime} and whether it was an environment variable to be mocked or simply the system clock.

To improve the design documentation for future implementations, we recommend adding a dedicated ``Data Types'' subsection to explicitly define the fields of custom objects like \texttt{UserProfile} to prevent integration mismatch. It would also be helpful to include pseudocode or step-by-step semantic descriptions for complex operations like \texttt{login}, clarifying exactly when to update internal state versus calling external providers. Finally, adding a column for specific exception trigger conditions---defining precisely \textit{why} a \texttt{TokenInvalid} exception should be thrown---would greatly aid in writing accurate unit tests.
