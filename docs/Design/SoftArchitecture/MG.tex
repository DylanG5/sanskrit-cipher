...
January 20 & 1.3 & Added sections 2-5\\
January 20 & 1.4 & Added sections 6-8\\
January 20 & 1.5 & Added sections 9-12\\
April 6 & 1.6 & Added low-fidelity user interface wireframes to support workflow planning and frontend implementation\\
\bottomrule
\end{tabularx}
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucIO}:] Input/Output devices
  (Input: Image files via web upload, Output: Screen display and file export). This is unlikely to change because the system is designed specifically for standard web-based input/output devices, and supporting alternative hardware would require a fundamental redesign.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucWebApp}:] The system will be
  deployed as a local application running on a user's machine. This is unlikely to change because the system is intended to be used locally, and changing the deployment model would significantly alter the architecture.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucImageProc}:] The system will
  process 2D images of manuscript fragments. This is unlikely to change because the system is specifically designed for 2D manuscript fragment analysis, and extending to other formats would require major redesign.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucCollaborate}:] The system will
  support collaborative work through shared projects and annotations. This is unlikely to change because collaboration is a core requirement of the system and removing it would contradict the primary use case.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucMLCore}:] Machine learning will
  be a core component for fragment matching and analysis. This is unlikely to change because the system fundamentally relies on machine learning techniques for reconstruction and analysis, and removing or replacing this would require a complete redesign of the system’s core functionality.
\end{description}

\section{Module Hierarchy} \label{SecMH}

This section provides an overview of the module design. Modules are summarized
in a hierarchy decomposed by secrets, shown in Table~\ref{TblMH}. The modules listed
...
\multirow{19}{0.3\textwidth}{Behaviour-Hiding}
& HW.ImageStorage \\
\cmidrule{2-2}
& \textit{User Interface}\\
& \quad UI.Canvas, UI.Search, UI.Auth, UI.Upload,\\
& \quad UI.SaveSession, UI.LoadSession\\
\cmidrule{2-2}
& \textit{Service Layer}\\
& \quad Svc.API, Svc.Search, Svc.AuthZ, Svc.Session,\\
& \quad Svc.ProcOrch, Svc.MatchOrch\\
\cmidrule{2-2}
& \textit{Data Management}\\
& \quad Data.FragmentStore, Data.Catalog,\\
& \quad Data.User, Data.Project\\
\cmidrule{2-2}
& \textit{Machine Learning}\\
& \quad ML.EdgeMatch, ML.ScriptClass, ML.TextSim,\\
& \quad ML.Segmentation, ML.CircleRecognition,\\
& \quad ML.LineCount, ML.EdgeClassification\\
\midrule
...
The \emph{Type of Module} field is used to explain why each module is shaped the way it is: \emph{Abstract Object} is used for stateful services, UI components, and ML components with hidden behaviour; \emph{Abstract Data Type} is used where operations must protect internal data representations; \emph{Record} is used for modules that primarily model structured stored entities; and \emph{Library} is used for shared, largely stateless helper functionality.
...
\item[Services:] Provides the required behaviour for storing and retrieving fragment images while hiding the underlying storage mechanism.
...
\node[module] (HH) at (1.5,-10) {M1\\ImageStore};
...
\begin{itemize}
\item \hyperref[mCanvas]{\textbf{Canvas (\mref{mCanvas})}}, \hyperref[mSearch]{\textbf{Search (\mref{mSearch})}}, \hyperref[mAuth]{\textbf{Auth (\mref{mAuth})}}, \hyperref[mUpload]{\textbf{Upload (\mref{mUpload})}}, \hyperref[mSaveSession]{\textbf{SaveSession (\mref{mSaveSession})}}, and \hyperref[mLoadSession]{\textbf{LoadSession (\mref{mLoadSession})}} all directly use \hyperref[mAPI]{\textbf{API (\mref{mAPI})}}
...
\item \hyperref[mSaveSession]{\textbf{SaveSession (\mref{mSaveSession})}} and \hyperref[mLoadSession]{\textbf{LoadSession (\mref{mLoadSession})}} provide dedicated interfaces for workspace persistence and retrieval
\end{itemize}

\subsubsection{Layer 4: API Gateway}
\textbf{API (M8)} serves as the central gateway and coordinates all service layer modules:
\begin{itemize}
\item Directly uses all five service modules: \hyperref[mSearchSvc]{\textbf{SearchSvc (\mref{mSearchSvc})}}, \hyperref[mAuthZ]{\textbf{AuthZ (\mref{mAuthZ})}}, \hyperref[mSession]{\textbf{Session (\mref{mSession})}}, \hyperref[mProcOrch]{\textbf{ProcOrch (\mref{mProcOrch})}}, and \hyperref[mMatchOrch]{\textbf{MatchOrch (\mref{mMatchOrch})}}
...
\begin{itemize}
\item \hyperref[mSearchSvc]{\textbf{SearchSvc (\mref{mSearchSvc})}} uses \hyperref[mFragStore]{\textbf{FragmentStore (\mref{mFragStore})}} and \hyperref[mCatalog]{\textbf{Catalog (\mref{mCatalog})}} to perform search and filtering operations across fragments
\item \hyperref[mAuthZ]{\textbf{AuthZ (\mref{mAuthZ})}} uses \hyperref[mUser]{\textbf{User (\mref{mUser})}} for credential verification and role-based authorization
\item \hyperref[mSession]{\textbf{Session (\mref{mSession})}} uses \hyperref[mUser]{\textbf{User (\mref{mUser})}} and \hyperref[mProject]{\textbf{Project (\mref{mProject})}} to manage user sessions and collaborative workspace state
\end{itemize}

\paragraph{Orchestration Services:}
\begin{itemize}
\item \hyperref[mProcOrch]{\textbf{ProcOrch (\mref{mProcOrch})}} orchestrates image processing pipelines by using:
  \begin{itemize}
  \item \hyperref[mFragStore]{\textbf{FragmentStore (\mref{mFragStore})}} for accessing fragment images
  \item \hyperref[mEdgeMatch]{\textbf{EdgeMatch (\mref{mEdgeMatch})}} for edge detection during preprocessing
  \item \hyperref[mScriptClass]{\textbf{ScriptClass (\mref{mScriptClass})}} for initial script classification
  \item \textbf{Segmentation (M21)} for isolating individual fragments from images
  \end{itemize}
\item \hyperref[mMatchOrch]{\textbf{MatchOrch (\mref{mMatchOrch})}} coordinates fragment matching by using:
  \begin{itemize}
  \item \hyperref[mEdgeMatch]{\textbf{EdgeMatch (\mref{mEdgeMatch})}} for edge profile comparison
  \item \hyperref[mScriptClass]{\textbf{ScriptClass (\mref{mScriptClass})}} for script similarity comparison
...
\begin{itemize}
\item \hyperref[mEdgeMatch]{\textbf{EdgeMatch (\mref{mEdgeMatch})}} uses \hyperref[mFragStore]{\textbf{FragmentStore (\mref{mFragStore})}} to access fragment images for edge detection and comparison
\item \hyperref[mScriptClass]{\textbf{ScriptClass (\mref{mScriptClass})}} uses both \hyperref[mCatalog]{\textbf{Catalog (\mref{mCatalog})}} for script taxonomy metadata and \hyperref[mFragStore]{\textbf{FragmentStore (\mref{mFragStore})}} for fragment images
...
\item Other ML modules (\hyperref[mTextSim]{\textbf{TextSim (\mref{mTextSim})}}) also use \hyperref[mFragStore]{\textbf{FragmentStore (\mref{mFragStore})}} for data access (connections shown representatively to reduce visual complexity)
...
\begin{itemize}
\item \hyperref[mFragStore]{\textbf{FragmentStore (\mref{mFragStore})}} uses:
  \begin{itemize}
  \item \hyperref[mHWImageStorage]{\textbf{HW.ImageStorage (\mref{mHWImageStorage})}} for file system and storage operations
  ...
  \end{itemize}
\item \hyperref[mCatalog]{\textbf{Catalog (\mref{mCatalog})}} uses \hyperref[mError]{\textbf{Error Handling (\mref{mError})}}, \hyperref[mLogging]{\textbf{Logging (\mref{mLogging})}}, and \hyperref[mConfig]{\textbf{Config (\mref{mConfig})}} for managing controlled vocabularies and metadata schemas
\item \hyperref[mUser]{\textbf{User (\mref{mUser})}} uses \hyperref[mError]{\textbf{Error Handling (\mref{mError})}}, \hyperref[mLogging]{\textbf{Logging (\mref{mLogging})}}, and \hyperref[mConfig]{\textbf{Config (\mref{mConfig})}} for user profile management and authentication data
\item \hyperref[mProject]{\textbf{Project (\mref{mProject})}} uses \hyperref[mError]{\textbf{Error Handling (\mref{mError})}}, \hyperref[mLogging]{\textbf{Logging (\mref{mLogging})}}, and \hyperref[mConfig]{\textbf{Config (\mref{mConfig})}} for collaborative workspace data
\end{itemize}
...
\begin{itemize}
\item \hyperref[mHWImageStorage]{\textbf{HW.ImageStorage (\mref{mHWImageStorage})}} abstracts file-system and storage operations, used by \hyperref[mFragStore]{\textbf{FragmentStore (\mref{mFragStore})}} for persistent storage
\item \hyperref[mError]{\textbf{Error Handling (\mref{mError})}} provides exception management and error reporting, used by all data modules and propagated throughout upper layers
\item \hyperref[mLogging]{\textbf{Logging (\mref{mLogging})}} records system events, user actions, and audit trails, used extensively by data and service layers
\item \hyperref[mConfig]{\textbf{Config (\mref{mConfig})}} manages application configuration from files and environment variables, used by all data modules and some services
\end{itemize}
...
The Sanskrit Manuscript Reconstruction Platform uses a client-server architecture in which the user interface communicates with backend services through a versioned service boundary. This section describes communication at the module-interface level, focusing on responsibilities and information flow rather than implementation syntax.

\subsection{API Architecture}

\subsubsection{Backend API Layer}
The backend API gateway provides a stable interface between UI modules and service modules. Its role is to route requests to the correct service module, enforce cross-cutting policies, and return standardized responses.

\textbf{API Design Principles:}
\begin{itemize}
  \item \textbf{Versioned Interface:} Service contracts are versioned to allow interface evolution without breaking existing clients.
  \item \textbf{Uniform Operations:} Interface operations are defined in terms of create, read, update, delete, and query actions over system resources.
  \item \textbf{Standardized Responses:} All operations return a consistent response structure with either requested data or a categorized error.
  \item \textbf{Input and Output Validation:} All module boundaries validate incoming arguments and outgoing results against agreed contracts.
  \item \textbf{Separation of Concerns:} Transport concerns are isolated in the API gateway while domain logic remains in service modules.
\end{itemize}

\subsection{Core API Capabilities}
At the architectural level, API capabilities are grouped by service responsibility rather than by concrete route definitions:
\begin{itemize}
  \item \textbf{Authentication and Authorization Operations:} establish and validate user identity, and enforce access permissions.
  \item \textbf{Fragment Lifecycle Operations:} ingest, retrieve, query, and remove fragment records and associated assets.
  \item \textbf{Preprocessing Operations:} initiate and monitor image normalization workflows.
  \item \textbf{Matching and Analysis Operations:} request candidate matches and retrieve ranked similarity outcomes.
  \item \textbf{Session and Workspace Operations:} create, update, restore, list, and remove saved workspace states.
\end{itemize}

\subsection{Data Exchange Formats}
Communication between modules uses typed request and response contracts with clear semantic fields.

\subsubsection{Request Contract Categories}
\begin{itemize}
  \item \textbf{Identity and Access Inputs:} credentials, session tokens, and authorization context.
  \item \textbf{Fragment Inputs:} image payload references and descriptive metadata required for storage and indexing.
  \item \textbf{Query Inputs:} search filters, sorting choices, and pagination parameters.
  \item \textbf{Workflow Inputs:} identifiers used to start or inspect preprocessing and matching jobs.
\end{itemize}

\subsubsection{Response Contract Categories}
\begin{itemize}
  \item \textbf{Operation Result:} requested resource data or confirmation of completed action.
  \item \textbf{Analysis Result:} ranked candidate lists with confidence or score summaries.
  \item \textbf{Workflow Status:} lifecycle state, progress indicators, and completion metadata.
  \item \textbf{Error Result:} normalized error category and diagnostic message suitable for UI handling.
\end{itemize}

\subsection{Inter-Service Communication}

\subsubsection{Backend to ML/AI Cluster}
The backend orchestration services communicate with ML components through internal service contracts (synchronous calls for immediate results and asynchronous jobs for long-running analysis):
\begin{itemize}
  \item \textbf{Text and Script Analysis:} model inference requests and prediction responses.
  \item \textbf{Similarity and Matching:} feature and score exchange for candidate ranking.
  \item \textbf{Long-Running Jobs:} queued execution with explicit status retrieval.
\end{itemize}

\subsubsection{Database Communication}
\begin{itemize}
  \item \textbf{Data Access Abstraction:} persistence is encapsulated behind data modules to isolate storage technology decisions.
  \item \textbf{Query Performance Support:} indexing and query planning are applied to satisfy search and retrieval constraints.
  \item \textbf{Concurrency Support:} data access is designed to support concurrent user workflows and collaborative use.
\end{itemize}

\subsection{Security and Data Integrity}

\begin{itemize}
  \item \textbf{Authentication:} Session and token-based identity verification.
  ...
  \item \textbf{Data Access Safety:} Protected persistence operations through validated and parameterized data access patterns.
...
\begin{itemize}
    \item \textbf{Behaviour-Hiding Module}
    \begin{itemize}
